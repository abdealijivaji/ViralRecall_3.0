...
Although several tools exist to identify giant viruses based on DNA sequences, these tools are generally broad-ranged and only include identification as a minor feature. Specifically, the tool `genomad` is an excellent resource to identify various phages and mobile genetic elements [@camargo:2024]. Since giant viruses are significantly different from most previously identified viruses, their dataset required special categories for giant virus identification. While `genomad` is an excellent resource, our tool is specifically designed to identify giant viruses, either present in metagenomes as independent contigs, or integrated into the genome of eukaryotes as GEVEs. Thus, we provide a more curated database and tool specifically designed to identify these enigmatic viruses.
